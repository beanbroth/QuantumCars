...
{
    [CustomEditor(typeof(MyLatticeDeformer))]
    public class MyLatticeDeformerEditor : DeformerEditor
    {
        ...
        protected override void OnEnable()
        {
            ...
            MyLatticeDeformer MyLatticeDeformer = ((MyLatticeDeformer) target);
            newResolution = MyLatticeDeformer.Resolution;
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Draw the control points array
            SerializedProperty controlPointsProperty = serializedObject.FindProperty("controlPoints");
            if (controlPointsProperty != null && controlPointsProperty.isArray)
            {
                EditorGUILayout.PropertyField(controlPointsProperty, new GUIContent("Control Points"), true);
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                myLatticeDeformer.GenerateControlPoints(newResolution, cachedResizePositions, cachedResizeResolution);
            ...
            if (GUILayout.Button("Reset Lattice Points"))
            {
                ...
                myLatticeDeformer.GenerateControlPoints(newResolution);
                selectedIndices.Clear();
        
                CacheResizePositionsFromChange();
            }

            if (myLatticeDeformer.CanAutoFitBounds)
            {
                if (GUILayout.Button("Auto-Fit Bounds"))
                {
                    Undo.RecordObject(myLatticeDeformer.transform, "Auto-Fit Bounds");
                    myLatticeDeformer.FitBoundsToParentDeformable();
                }
            }
        ...
        public override void OnSceneGUI()
        {
            ...
            MyLatticeDeformer lattice = target as MyLatticeDeformer;
            ...
            using (new Handles.DrawingScope(transform.localToWorldMatrix))
            {
                ...
                for (int z = 0; z < resolution.z; z++)
                {
                    for (int y = 0; y < resolution.y; y++)
                    {
                        ...
                        {
                            var controlPointHandleID = GUIUtility.GetControlID("MyLatticeDeformerControlPoint".GetHashCode(), FocusType.Passive);
        ...
        private void CacheOriginalPositions()
        {
            ...
            var MyLatticeDeformer = (target as MyLatticeDeformer);
            float3[] controlPoints = MyLatticeDeformer.ControlPoints;
        ...
        private void CacheResizePositionsFromChange()
        {
            ...
            cachedResizeResolution = MyLatticeDeformer.Resolution;
        }
        ...
        private void DrawLattice(MyLatticeDeformer lattice, DeformHandles.LineMode lineMode)
        {
            var resolution = lattice.Resolution;
